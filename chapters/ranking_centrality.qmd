...
:::{.callout-caution}
Because $\mathbf{A}$ is not symmetric, we can also define $k_j^{\mathrm{in}} = \sum_{j \in N}a_{ij}$, the number of incoming links to node $j$. Importantly, $k_j^{\mathrm{in}} \neq k_j^{\mathrm{out}}$ in general. 
:::
